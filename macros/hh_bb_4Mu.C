...
#include "TFile.h"
#include <TLatex.h>
//#include "/home/aya/programs/Delphes/Delphes-3.4.2/classes/DelphesClasses.h"
#include <iostream>
#include <iomanip>
#include <fstream>
#include <string>
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   // TH1D
   // various muon combinations bet 1,2,3,4 muons gives various Z masses, choose combination wich is nearer to Z mass 91 GeV, when you find 2 muon pairs with mass ~ 91 GeV take them as two Z of signal and call them Za, Zb
   ...
   // Eta MET
   h_eta_MET = new TH1D("h_eta_MET", "h_eta_MET", 20, -8., 8.);
   ...
   // Eta of all Muons
   h_eta_allMuons = new TH1D("h_eta_allMuons", "h_eta_allMuons", 20, -8., 8.);
   ...
   // Eta of all Jets
   h_eta_allJets = new TH1D("h_eta_allJets", "h_eta_allJets", 20, -8., 8.);
   ...
   h_eta_Za = new TH1D("h_eta_Za", "h_eta_Za", 20, -8., 8.); 
   ...
   h_eta_Zb = new TH1D("h_eta_Zb", "h_eta_Zb", 20, -8., 8.); 
   ...
   h_eta_h1_ZaZb = new TH1D("h_eta_h1_ZaZb", "h_eta_h1_ZaZb", 20, -8., 8.);
   ...
   h_eta_h1_b1b2 = new TH1D("h_eta_h1_b1b2", "h_eta_h1_b1b2", 20, -8., 8.);
   ...
   // Define data files
   TFile *input_file = new TFile("/media/aya/AYA.HDD/MGSamples2020/DY_BG_14TeV_SMfull_GEN/DY_BG_14TeV_SMfull_pythia8_CMSPhaseII-0PU_GEN-SIM.root", "READ");
  ...
  //Lmc = nb_events/cross-section;
  // Lmc for process: DY at 14TeV wrto xsection(fb)
  ...
  std::cout << "INFO: Start looping over all Events!" << endl;  
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      // loop overall Muons, jets and MET to get their size
      for (Int_t i = 0; i < MuonLoose_size; i++){
		  h_muons_size->Fill(i);
		  h_pt_allMuons->Fill(MuonLoose_PT[i], wt);
		  h_eta_allMuons->Fill(MuonLoose_Eta[i], wt);
		  h_phi_allMuons->Fill(MuonLoose_Phi[i], wt);
      }
   ...
   h_muons_size->Write();
   h_pt_allMuons->Write();
   h_eta_allMuons->Write();
   h_phi_allMuons->Write();
   ...
   std::cout << "Drawing Histograms!" << endl;
   
   h_muons_size->Draw("CE1");
   h_pt_allMuons->Draw("CE1");
   h_eta_allMuons->Draw("CE1");
   h_phi_allMuons->Draw("CE1");
    
   h_jets_size->Draw("CE1");
   h_pt_allJets->Draw("CE1");
   h_eta_allJets->Draw("CE1");
   h_phi_allJets->Draw("CE1");
   ...
   std::cout << "ROOT file:" << output_file << " has been created sucessfully!" << endl; 
