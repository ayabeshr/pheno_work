...
#include "/home/aya/Pheno_Work/analysis/macro/newupdate/DemoAnalyzer.h"
#include "/home/aya/Pheno_Work/analysis/macro/newupdate/newtree.h"
...
void DemoAnalyzer::Loop()
{
   ...
   //TFile *out_sig = new TFile("/home/aya/Pheno_Work/analysis/output_FullSel_cutflow2/out_sig3_1e04Events_unweighted_FullSel_cutflow2.root", "RECREATE");
  ...
  /*==================================================================================*/ 
   ...
   for (Long64_t jentry = 0; jentry < nentries; jentry++)  
   {
	...
	met.met_phi.push_back(MET_phi);
	 ...
	 double m23 = (mu2 + mu3).M(); */
         ...
         Float_t mZ1, mZ2, ptZ1, ptZ2, etaZ1, etaZ2, phiZ1, phiZ2;
	     ...
	     // 4th Selection: on charge of each muon pair
	 
	       if ( MuonTight_Charge[mu1_idx] + MuonTight_Charge[mu2_idx] == 0){  // mu1, mu2 
				   
                  if ( MuonTight_Charge[mu3_idx] + MuonTight_Charge[mu4_idx] == 0){  // mu3, mu4

		     NEvents[3]++;


		     // 5th Selection: on Impact Parameters of 4Muons, all Muons of signal should come from PV 
			  ...
			  && (dxy_mu3 < 0.12) && (dxy_mu4 < 0.12) ){
			   ...
			   NEvents[4]++;
                          ...
                          if ( ( pt_mu1 > 35. ) && ( pt_mu2 > 20. ) ){         
                              ...
                              // 7th Selection: on DR between muons for each pair
			      
			      if ( (DR_Tight_mu12 > 0.07) && (DR_Tight_mu34 > 0.07) ){

				 NEvents[6]++;

				 // 8th Selection: on Mass of each muon pair 

				 if ( (m12 > 3.) && (m34 > 3.) ){

				    NEvents[7]++;

				    // plot 4Muons kinematics after above selections ( acc to plots tighter cuts may be applied instead later)
				    ...
				    drMuonsTight.v_DR_mu1mu2.push_back(DR_Tight_mu12);
                                    ...
                                    // plot Z1,Z2 invariant mass with no Z window specification
				    tight_mu.tight_mu12_mass.push_back(m12);
                                    tight_mu.tight_mu34_mass.push_back(m34);
				    */
				    

				    // Build Z1,Z2 candidates
				    ...
				    mZ2   = Z2.M();   // invariant mass of muons 3&4
				    ptZ2  = Z2.Pt();
                                    etaZ2 = Z2.Eta();
				    phiZ2 = Z2.Phi();
					...
					jets.jet_pt.push_back(j_pt);
				     ...
				     jets.loose_bjet_size.push_back(nbjets);
					  ...
					  NEvents[12]++;  // nb of events passed selections till bjet pt, eta, DR selections
				 ...
				 } // mass of mu pair 12 & 34 > 3  
			      } // DR for mu pairs 12 & 34
			  } // pt of lead & sublead mu 
		      } // dz  
		  } // dxy 
	       } // charge of mu pair 34 = 0
	    } // charge of mu pair 12 = 0
	 }  // total charge of 4 muons = 0
   ...
   cout << "================================================================================"    << endl;
   cout << " muon pT > 5GeV & |eta| < 2.7                            |   " <<  NEvents[0]        << endl;
   cout << "================================================================================"    << endl;
   cout << " number of muons/event > 3                               |   " <<  NEvents[1]        << endl;
   cout << "================================================================================"    << endl;
   cout << "Total 4 muon charge/event = 0                            |   " <<  NEvents[2]        << endl; 
   cout << "================================================================================"    << endl;
   cout << " charge of mu pairs 12 & 34 = 0                          |   " <<  NEvents[3]        << endl; 
   cout << "================================================================================"    << endl;
   cout << " IP: Dxy < 0.12 & DZ < 0.47                              |   " <<  NEvents[4]        << endl; 
   cout << "================================================================================"    << endl;
   cout << " lead & sublead mu pT > 20 & > 10 respectively           |   " <<  NEvents[5]        << endl; 
   cout << "================================================================================"    << endl;
   cout << " DR for mu pairs 12 & 34 > 0.07                          |   " <<  NEvents[6]        << endl; 
   cout << "================================================================================"    << endl;
