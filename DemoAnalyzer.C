   ...
   for (Long64_t jentry = 0; jentry < nentries; jentry++) 
   {
	...
	Long64_t ientry = LoadTree(jentry);
	  ...
	  for (Int_t i = 0; i < Particle_size; i++){
	      ...
	      int muonL_size = MuonLoose_size;  // variable saves number of muons/event 
         
          
          // 1st Selection: on number of Muons per event
          if ( muonL_size > 3 ){ // having at least 4 Muons per event 
	      ...
	      // TLorentzVector declarations 
             ...
             /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
              ...
              // 2nd Selection: on 4Muons charge 
